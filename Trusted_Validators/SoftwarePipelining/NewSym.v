...
Proof. 
  ...
  assert (of_value_list ge (get_args args (file t)) s0 = Some rs ## args).  
    assert (rs = (mkst stack c sp rs m).(NewSE.rs)). trivial. rewrite H2. 
 ...
 exists v. rewrite H2. generalize (sem_id _ _ _ _ H0); intro. intuition. simpl in *|-; subst; trivial. 
  ...
  inversion H1; subst; try congruence. inversion H2; subst. clear H2. 
  assert (of_value_list ge (get_args args (file t)) s0 = Some rs ## args).  
    assert (rs = (mkst stack c sp rs m0).(NewSE.rs)). trivial. rewrite H2. 
    eapply two_aux_aux_list; eauto. 
  ...
  inversion H1; subst; try congruence.
    assert (of_value_list ge (get_args args (file t)) s0 = Some rs ## args).  
    assert (rs = (mkst stack c sp rs m0).(NewSE.rs)). trivial. rewrite H5. 
    eapply two_aux_aux_list; eauto. 
...
Axiom domain_prop_1: forall o a b, domain a o -> domain b o -> domain (a ^^ b) o. 
...
Proof. 
  ...
  simpl. rewrite H6. rewrite H5. eapply P.union_equal_2; eauto. 
  unfold substitute_value_ext. 
  repeat rewrite Constraints.fold_1.
...
Admitted. (* eq_to_sem_aux2 : a l air vrai, preuve par induction multiple *)  
...
Proof. 
  ...
  simpl in H6; subst. 
  simpl in H3. 
Admitted. (* eq_to_sem_aux3 : a l air vrai, preuve par induction multiple *)  
...
Parameter obs_incl : symbolic_value -> observables -> bool.  
 
Definition eq_sv_opt r t1 t2 obs : bool :=
  match ((file t1) ! r), ((file t2) ! r) with
  | Some v1, Some v2 => if eq_dec_sv v1 v2 && obs_incl v1 obs && obs_incl v2 obs then true else false
  | None, None => true 
  | _, _ => false
  end. 

Definition check o (t1 t2 : sym_state) : bool :=
   List.fold_left (fun b obs => b && eq_sv_opt obs t1 t2 o)  o true 
  && (if eq_dec_sm t1.(mem) t2.(mem) then true else false) 
  && (Constraints.equal t1.(cons) t2.(cons)).
(* ICICICICICI*)
Lemma check_file_correct :
  forall t1 t2 o x,
  fold_left (fun (b : bool) (obs : positive) => b && eq_sv_opt obs t1 t2 o) o true = true ->
  In x o ->
  (file t1) ! x = (file t2) ! x.
Proof. 
  induction o; intros. 
  inversion H0. 
  generalize (peq x a); intros. 
  destruct H1. 
  subst. 
  simpl in H. 
  assert (eq_sv_opt a t1 t2 o = true).  admit. (* OK FOLD check*)
  unfold eq_sv_opt in H1. 
  case_eq ((file t1) ! a); intros. rewrite H2 in H1. 
  case_eq ((file t2) ! a); intros. rewrite H3 in H1. 
  case_eq (eq_dec_sv s s0); intros. clear H4. subst. 
  trivial. 
  rewrite H4 in H1. inversion H1. 
  rewrite H3 in H1. inversion H1. 
  rewrite H2 in H1. 
  case_eq ((file t2) ! a); intros.
  rewrite H3 in H1. inversion H1. 
  trivial. 
  rewrite IHo. 
  trivial. simpl in H. admit. (*OK FOLD IDEM check*)
  inversion H0. congruence. 
  trivial. 
Qed. 

Lemma check_correct : 
  forall obs t1 t2, check obs t1 t2 = true -> weq obs t1 t2. 
Proof.
  intros. 
  unfold check in H. 
  case_eq (fold_left (fun (b : bool) (o : positive) => b && eq_sv_opt o t1 t2 obs) obs true); intros.
  ...
  eapply Seq_weak; eauto.
  intros. 
  eapply check_file_correct; eauto. 
...
Definition weq := weq.  
Definition rewrite_left := rewrite_left. 
Definition rewrite_right := rewrite_right.
Definition dec := check. 
Definition dec_correct := check_correct. 
Lemma correct :  forall o l l', weq o (evaluate l) (evaluate l') -> forall ge s s', lsteps ge l s s' -> exists s'', lsteps ge l' s s'' /\ state_eq o s' s''. 
Proof. 
  intros. 
